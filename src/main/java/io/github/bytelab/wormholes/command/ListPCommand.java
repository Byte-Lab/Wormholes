...
import me.tbotv63.core.util.command.ArgumentType;
...
public class ListPCommand extends BaseCommand {

    public ListPCommand() {
        specification = new Specification(ArgumentType.INTEGER);
        aliases.add("list");
        aliases.add("l");
    }
    ...
    public boolean execute(CommandSender commandSender, Command command, String s, String[] args) throws InsufficientPermissionException {
        ...
        int pageid = Integer.parseInt(args[0]) - 1;

        commandSender.sendMessage(LocaleManager.get("message.list.heading", pageid + 1));

        for (int i = pageid * Config.pageLength; i < (pageid * Config.pageLength) + Config.pageLength; i++) {
            if (i >= c.size()) { break; }
