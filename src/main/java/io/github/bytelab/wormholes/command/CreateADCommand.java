...
public class CreateADCommand extends BaseCommand {

    public CreateADCommand() {
        specification = new Specification(STRING, FLOATING, FLOATING, FLOATING);
        aliases.add("c");
        aliases.add("create");
    }
    ...
    public boolean execute(CommandSender commandSender, Command command, String s, String[] args) throws InsufficientPermissionException {
        ...
        if (! (commandSender instanceof Player)) {
            ...
            Wormhole wormhole = new Wormhole(position, sender.getWorld(), args[0], destination);

            Main.getInstance().getDestinations().putElement(destination);
            Main.getInstance().getWormholes().putElement(wormhole);
